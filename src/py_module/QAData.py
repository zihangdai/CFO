...
class QAData(object):
    ...
    #     # sub_score = preprocessing.scale(np.sum(self.score_mat, 1))
    #     # sub_score += self.ent_score
    #     sub_score = np.sum(self.score_mat, 1)

    #     top_subid = np.argmax(sub_score)
    #     top_relid = np.argmax(self.score_mat[top_subid])
    #     self.pred_sub = self.cand_sub[top_subid]
    #     self.pred_rel = self.cand_rel[top_relid]
    #     return self.cand_sub[top_subid], self.cand_rel[top_relid]
